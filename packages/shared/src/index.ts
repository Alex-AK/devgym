 ...
 */
/**
 * Every kind a session may author. `module` sat here as a reserved kind the
 * loader refused, from before modules existed until the day they did, which
 * cost exactly the one case in a switch it was reserved to cost. Nothing is
 * reserved now, so there is one list rather than two.
 */
