 ...
 * table in the same database now, and the `async` was the last thing left over
 * from the old one: better-sqlite3 never returns a promise, and a function that
 * does cannot run inside a transaction.
 */
export function recordDecision(db: Db, entry: LogEntry): void {
  db.prepare(
