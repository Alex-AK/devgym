 ...
 * resolves only when `advance` moves the clock past its deadline, so a
 * thirty-second visibility timeout costs a checkpoint nothing to expire.
 *
 * Given to you, and not part of the exercise. The queue reads its deadlines
 * from `now()`; use `now()` and `sleep()` yourself, and leave `advance()` to the
 * checkpoints.
 */
export class Clock {
  private nowMs = 0;
